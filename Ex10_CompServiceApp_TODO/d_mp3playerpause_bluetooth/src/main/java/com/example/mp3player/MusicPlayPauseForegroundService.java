...
public class MusicPlayPauseForegroundService extends Service {
    ...
    // 블루투스 기기
    public static final String ACTION_TOGGLE_PLAY_PAUSE ="com.example.mp3player.ACTION_TOGGLE_PLAY_PAUSE";
    ...
    /**
     * TODO 2.
     * 미디어 버튼으로부터 요청이 왔을 때, 현재 재생 상태에 따라 재생/재개 또는 일시정지를 토글합니다.
     */
    private void handleTogglePlayPause() {
        // player가 존재하고 재생 중인 경우 -> 일시정지
        if (player != null && player.isPlaying()) {
            handlePause();
        } else {
            // player가 null이거나 일시정지 상태인 경우 -> 재생/재개
            handleStartResume();
        }
        Log.i(TAG, "미디어 버튼 토글 요청 처리됨.");
    }
